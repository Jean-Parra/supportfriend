import { products, filterProdutsByPrice, filterProdutsByText } from './prosthesis.js';
import { replaceScreenProduct } from './pagination_prosthesis.js';
...
for (let i = 0; i < products.length; i++) {
	min_price = Math.min(
		min_price,
		parseInt(products[i].price.replaceAll('.', '')),
	);
	max_price = Math.max(
		max_price,
		parseInt(products[i].price.replaceAll('.', '')),
